...
/// Base language to use as the canonical reference for required strings.
let baseLang = "en"
...
func main() throws {
    guard CommandLine.arguments.count == 2 else {
        die("""
            Usage: \(CommandLine.arguments.first!) <searchRoot>
            The searchRoot will be scoured for .strings files. (Some sub-paths will be excluded automatically, including $PODS_ROOT and the build products directory.)
            """)
    }
    
    let searchRoot = CommandLine.arguments[1]
    ...
    guard let builtProductsDir = ProcessInfo().environment["BUILT_PRODUCTS_DIR"] else {
        die("$BUILT_PRODUCTS_DIR is not set in the environment. Are you sure Xcode invoked this script?")
    }
